...
function RequestAdmin() {
    ...
    const [targetRequesId, setTargetRequestId] = useState(null);
    ...
    return <>
        <div className="requestAdminContainer">
            ...
            <table>
                ...
                <tbody>
                    {requests.map((request) => <Request key={request.Id} request={request} setTargetRequestId={setTargetRequestId} setEquipments={setEquipments}/>)}
                </tbody>
            </table>
            ...
            <div>
                <h3>Description</h3>
                <p>{requests.length>0 && requests.filter(request=>request.Id==targetRequesId)[0]?.Description}</p>
                <div className="requestAdmin_equipmentList">
                    <table>
                        ...
                        {equipments.map((equipment) => (
                            <Equipment key={equipment.id} equipment={equipment} setEquipmentInfo={setEquipmentInfo}/>))}
                        </tbody>
                    </table>
...
function Request({request, setEquipments, setTargetRequestId}) {
    ...
    async function approveRequest(requestID) {
        try {
            const response = await fetch("http://localhost:8080/requests/changestatus/"+requestID, {
                method: 'PUT',
                headers: {
            ...
            if (response.ok) {
                ...
                alert("Budget Is Not Sufficient");
